...
from udp_1028 import Udp1028

class Udp1028Parser(ParserBase):
    def __init__(self, read_file, show_pbar=False, log_level=logging.INFO):
        super().__init__(read_file, protocol='udp_1028', show_pbar=show_pbar, log_level=log_level)
    ...
    def process_capture(self, capture):
        ...
        while stream.pos() < len(capture):
            ...
            try:
                udp1028 = Udp1028(stream)
            except Exception as e:
                self.logger.debug(f"Parse error at {pos}: {e}")
                self.consume_byte(stream, pos, pbar)
            else:
                self.logger.header(f"{udp1028.magic}, {udp1028.seq}")
                udp1028_bytes = stream._io[pos:stream.pos()]
                self.logger.debug(f"{udp1028_bytes[2:]}")
                self.write_protocol.write(udp1028_bytes[2:])
                # stream.seek(stream.pos())
                SKIPPED_BYTES = stream._io[prev_frame_end:pos]
                self.skips[prev_frame_end] = pos
                prev_frame_end = stream.pos()
                self.consume_bytes(len(udp1028_bytes), pbar)
    ...
    def log_status(self, logger=None):
        ...
        # udp_1028 log custom performance metrics here


def main():
    parser = argparse.ArgumentParser(description="Process udp_1028 capture files with custom logging verbosity.")
    ...
    udp_1028 = Udp1028Parser(capture_file, log_level=log_level, show_pbar=True)
    udp_1028.process_capture_file(capture_file)
    udp_1028.log_status()
    udp_1028.done_processing()
    udp_1028.plot_skips()
